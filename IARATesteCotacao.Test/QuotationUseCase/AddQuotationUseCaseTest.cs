...
{
    public class AddQuotationUseCaseTest
    {
        [Fact]
        public async void Create_New_Quotation_All_Fields()
        {
            ...
            quotationRepo.Setup(x => x.AddAsync(It.IsAny<Quotation>())).Returns(Task.FromResult(quotatioTest));
            var handler = new AddQuotationHandler(quotationRepo.Object, localityService.Object);
            var result = (ApiResult<Quotation>)await handler.Handle(new AddQuotationCommand
            {
